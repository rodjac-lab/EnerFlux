  ...
  for (let day = 0; day < totalDays; day += 1) {
    ...
    for (let offset = -toleranceSteps; offset <= toleranceSteps; offset += 1) {
      ...
      if (Math.abs(sampleTime_s - targetTime_s) <= tolerance_s) {
        ...
        const distance = Math.abs(sampleTime_s - targetTime_s);
        const isCloser = distance + EPSILON < bestDistance;
        const isSameDistanceBetter =
          Math.abs(distance - bestDistance) <= EPSILON && value > observedTemp;
        if (isCloser || isSameDistanceBetter) {
          observedTemp = value;

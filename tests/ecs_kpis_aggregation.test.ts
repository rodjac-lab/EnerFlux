...
describe('Agrégation journalière des KPI ECS', () => {
  ...
  it("choisit l'échantillon le plus proche de la deadline plutôt que la valeur maximale", () => {
    const dt_s = 900; // 15 min
    const hoursPerDay = 24;
    const stepsPerDay = (hoursPerDay * 3600) / dt_s;
    const temps = new Array<number>(stepsPerDay).fill(48);
    const deadlineHour = 21;
    const deadlineIndex = Math.round((deadlineHour * 3600) / dt_s);

    temps[deadlineIndex - 1] = 60; // pic avant la deadline
    temps[deadlineIndex] = 50; // température réelle à la deadline
    temps[deadlineIndex + 1] = 48; // chute après la deadline

    const penaltyPerKelvin = 0.2;
    const aggregation = aggregateEcsDeadlineKpis({
      temps_C: temps,
      dt_s,
      targetCelsius: 55,
    ...
    expect(aggregation.daily).toHaveLength(1);
    const day = aggregation.daily[0];
    expect(day.observedTemp_C).toBeCloseTo(50, 6);
    expect(day.deficit_K).toBeCloseTo(5, 6);
    expect(day.hit).toBe(false);
    expect(day.penalty_EUR).toBeCloseTo(5 * penaltyPerKelvin, 6);
  });
});
